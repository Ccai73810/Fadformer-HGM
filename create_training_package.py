...
# Setup paths
base_dir = r'd:\NeRD-Rain\FADformer'
pkg_dir = os.path.join(base_dir, 'HGM_Training_Package')

# 1. Create target directories
os.makedirs(pkg_dir, exist_ok=True)
os.makedirs(os.path.join(pkg_dir, 'models'), exist_ok=True)
os.makedirs(os.path.join(pkg_dir, 'datasets'), exist_ok=True)
os.makedirs(os.path.join(pkg_dir, 'utils'), exist_ok=True)

# 2. Copy core model files
shutil.copy(os.path.join(base_dir, 'models', '__init__.py'), os.path.join(pkg_dir, 'models', '__init__.py'))
...
shutil.copy(os.path.join(base_dir, 'datasets', 'Rain_Dataloader.py'), os.path.join(pkg_dir, 'datasets', 'Rain_Dataloader.py'))

# 4. Copy utility files
shutil.copy(os.path.join(base_dir, 'utils', '__init__.py'), os.path.join(pkg_dir, 'utils', '__init__.py'))
shutil.copy(os.path.join(base_dir, 'utils', 'common.py'), os.path.join(pkg_dir, 'utils', 'common.py'))
shutil.copy(os.path.join(base_dir, 'utils', 'utils.py'), os.path.join(pkg_dir, 'utils', 'utils.py'))
shutil.copy(os.path.join(base_dir, 'utils', 'data_parallel.py'), os.path.join(pkg_dir, 'utils', 'data_parallel.py'))
...
readme_content = """# FADformer-HGM 独立训练与部署包
...
# 8. Compress to ZIP file
zip_path = os.path.join(base_dir, 'HGM_Training_Package.zip')
with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
    for root, dirs, files in os.walk(pkg_dir):
        for file in files:
            ...
            zipf.write(file_path, os.path.join('HGM_Training_Package', arc_name))

print("SUCCESS: Standalone training package successfully created!")
